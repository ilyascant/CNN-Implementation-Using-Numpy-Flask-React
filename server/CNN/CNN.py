    ...
    def train(self, epochs=10, batch_size=1024, conv_stride=1, pool_size=2, pool_stride=2, beta1=0.9, beta2=0.999, patience=3):
        ...
        for epoch in range(epochs):
            ...
            for i in t:
                ...
                t.set_description(f"Computing validation coss {val_cost/(i+1):.4f}")
            ...
            if (epochs_no_improve >= patience):
                with open(f'cnn_model_L{best_val_cost:.8f}_R{int(random.random() * 1e8)}'.replace('.',"-")+'.pkl', 'wb') as file:
                    pickle.dump(best_cnn, file)
                    epochs_no_improve = 0
                    best_val_cost = float('inf')
